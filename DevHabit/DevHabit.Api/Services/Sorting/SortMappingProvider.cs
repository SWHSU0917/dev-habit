...
{
    public SortMapping[] GetMappings<TSource, TDestinatiion>()
    {
        SortMappingDefinition<TSource, TDestinatiion>? sortMappingDefinition = sortMappingDefinitions
            .OfType<SortMappingDefinition<TSource, TDestinatiion>>()
            .FirstOrDefault();

        if (sortMappingDefinition is null)
        {
            throw new InvalidOperationException(
                $"The mapping from '{typeof(TSource).Name}' into '{typeof(TDestinatiion).Name} isn't defined");
        }
